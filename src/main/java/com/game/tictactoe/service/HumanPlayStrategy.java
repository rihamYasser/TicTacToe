...
public class HumanPlayStrategy implements PlayStrategy {
    ...
    private Position formatInput(String input) {
        ...
        if (indexes.length <= 1) {
            throw new InvalidInputException("Please enter a valid position in a format like 3,1");
        }
        try {
            return new Position(Integer.parseInt(indexes[0]), Integer.parseInt(indexes[1]));
        } catch (NumberFormatException ex) {
            throw new InvalidInputException("Please enter valid row and column numbers");
        }
    }
}
